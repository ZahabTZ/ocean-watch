import { CATEGORY_META, ComplianceAlert, getComplianceAlerts, getFleetVessels, getRfmoSources } from '@/data/liveData';
...
const FLEET_PROFILE = {
  ...
  vesselNames: getFleetVessels().map(v => v.name),
};
...
export function queryKnowledgeBase(query: string): QueryResult {
  ...
  if (isWeeklyQuestion(lower) && relevantAlerts.length === 0) {
    const actionAlerts = alerts.filter(a => a.status === 'action_required');
  ...
  if (lower.includes('source') || lower.includes('rfmo') || lower.includes('monitor')) {
    ...
    return {
      answer: `**RFMO Monitoring Status:** ${online}/${sources.length} sources online.\n\n${sources.map(s => `• **${s.acronym}** (${s.region}): ${s.status === 'online' ? '🟢' : '🟡'} ${s.status} — ${s.documentsIngested} docs ingested, last checked ${s.lastChecked}`).join('\n')}`,
      sources: [],
    };
  }
